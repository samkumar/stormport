interface SocketSpi
{
    ...
    command void writeRegister(uint16_t reg_addr, uint8_t *buf, uint16_t len);
    ...
    command void readRegister(uint16_t reg_addr, uint8_t *buf, uint16_t len);

    // signaled when write or read is finished. If the command was read,
    // then the results will be stored at *buf with length len
    event void taskDone(error_t, uint8_t *buf, uint16_t len);
}
